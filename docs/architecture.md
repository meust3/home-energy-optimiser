...
The production deployment is Home Assistant App v0.4.1 (formerly called an add-on)
on the amd64 Home Assistant OS 18.1 NUC, where it is collecting successfully at
PostgreSQL revision `20260811_01`. Version 0.4.0 introduced that explicit additive
database migration, and v0.4.1 added schema-neutral power-sign configuration.
Supervisor injects `SUPERVISOR_TOKEN`; the App uses it only as a
